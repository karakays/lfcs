...
[ XXXIX. SysV, upstart and systemd](#xxxix-sysv-upstart-and-systemd)
...
You see that kernel command line and its boot parameters are specified in `GRUB_CMDLINE_LINUX` and it is just one part of the whole `GRUB` configuration. There are many others like `GRUB_TIMEOUT`, `GRUB_TERMINAL`, `GRUB_INIT_TUNE` and some others which come from `/etc/grub.d/`
...
Modern systems use `upstart` or `systemd` which enables parallel system startup. `init` is the general name (like it's interface and there are multiple implementations)
...
You can change current run level with `telinit`.

`telinit 5`

To get the current runlevel, run where `N` is the previous level meaning unknown.
...
All service scripts are located in `/etc/init.d`. To create a new `SysV` service, its script needs to be put in `/etc/init.d` directory. Scripts from runlevel directories link back to `/etc/init.d`. 

* Startup script links start with `S` in the name wheresa kill script links with `K`.
* The number following the script type letter determines execution order.

```
$ ll /etc/rc5.d/
lrwxrwxrwx 1 root S01ssh -> ../init.d/ssh
lrwxrwxrwx 1 root S01rsync -> ../init.d/rsync
lrwxrwxrwx 1 root S01lightdm -> ../init.d/lightdm
...
```

`/etc/rc.local` contains custom startup scripts that is run after a multiuser runlevel scripts has completed.

##### `chkconfig`
Used to check if a system is running, to start/stop a service during boot (enable/disable) etc.

##### `service`
Used to manage services, start, stop, status, reload etc. It's `systemctl` equivalent in `systemd`.

#### `upstart`
`upstart` is event-driven. Events are sent to `init` process to tell it to execute certain commands. `initctl` is `systemctl` equivalent in `systemd`.
